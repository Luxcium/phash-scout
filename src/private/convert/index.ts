...
const path_ = '';
const normalPath = normalize(path_);
const parsedNormalPath = parse(normalPath);
parsedNormalPath;

export const getUnExtPath = (normal: path.ParsedPath) =>
  `${normal.dir}${path.sep}${normal.name}`;

const extName = path.extname(normalPath).toLowerCase();
if (extName === '.webp') {
  const jpegNormalPath = normalize(`${getUnExtPath(parsedNormalPath)}.jpeg`);
  jpegNormalPath;

  const convert = spawn('convert', [normalPath, jpegNormalPath]);

  convert.stdout.on('data', (data: any) => {
