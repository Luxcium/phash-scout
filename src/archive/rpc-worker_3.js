...
export class RpcWorkerPool {
  ...
  constructor(path, size = 0, strategy = 'roundrobin') {
    ...
    for (let i = 0; i < this.size; i++) {
      ...
      worker.on('message', msg => {
        this.onMessageHandler(msg, this.worker_uid);
      });
    }
  }
  // ++ ----------------------------------------------------------------
  ...
  getWorker() {
    ...
    if (this.strategy === 'random') {
      id = Math.floor(Math.random() * this._workers.size);
    } else if (this.strategy === 'roundrobin') {
      this.rr_index++;
      if (this.rr_index >= this._workers.size) this.rr_index = 0;
      id = this.rr_index;
    } else if (this.strategy === 'leastbusy') {
      let min = Infinity;
      for (let i = 0; i < this._workers.size; i++) {
        let worker = this._workers.get(i);
