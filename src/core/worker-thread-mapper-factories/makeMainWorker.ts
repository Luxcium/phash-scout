...
export function makeMainWorker<T>(
  filename: string,
  worker_threads: Worker_Threads<WorkerData<T>>): <R>(value: T, index?: number, array?: readonly T[]) => Promise<R> {
  return <R>(value: T, index?: number, array?: readonly T[]): Promise<R> => worker_threads.isMainThread
